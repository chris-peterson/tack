...
function formatTackDetails(tack, indent) {
    const lines = [];
    if (tack.deliverable) {
...
function aspectValue(tack, aspect) {
    switch (aspect) {
        case "deliverable":
...
function aspectPresent(value) {
    return Array.isArray(value) ? value.length > 0 : value !== null;
}
function globData(routes, path) {
        ...
        for (const r of matchedRoutes) {
            for (const tack of r.tacks.filter((t) => globMatch(tackPat, t.id))) {
                for (const aspect of matchedAspects) {
                    ...
                    if (aspectPresent(value))
                        push({ slug: r.slug, tackId: tack.id, aspect, value });
                }
            }
        }
    }
 ...
 */
export function treeData(routes, path) {
    ...
    if (path?.includes("/")) {
        ...
        if (tackId) {
            ...
            if (aspect) {
                ...
                return { [aspect]: aspectValue(tack, aspect) };
            }
            return tack;
        }
        path = slug;
...
function expandDoublestar(parts) {
    if (!parts.some((p) => p === "**"))
        return [parts];
    const results = [];
...
function resolveGlobParts(routes, parts) {
    const [slugPat, tackPat, aspectPat] = parts;
    const lines = [];
    const matchedRoutes = routes.filter((r) => globMatch(slugPat, r.slug));
    if (parts.length === 1) {
...
function resolveGlob(routes, path) {
...
export function formatTree(routes, path, depth) {
    if (path && hasGlob(path)) {
...
export function formatRecent(routes) {
    if (routes.length === 0) {
...
export function formatFind(matches) {
    if (matches.length === 0) {
...
export function formatList(routes) {
    if (routes.length === 0) {
        return "No routes found.";
    }
    const lines = [];
