...
export function shouldAnnounce(env = process.env) {
...
export function announce(key, fields, opts = {}) {
    const env = opts.env ?? process.env;
    if (!shouldAnnounce(env))
        return false;
...
export function announceOnce(key, sessionId, fields, opts = {}) {
    const env = opts.env ?? process.env;
    if (!shouldAnnounce(env))
        return false;
