...
describe("tack start auto-binds the current Claude session (beacon fleet join)", () => {
    it("records the started tack on the session", () => {
        ...
        assert.doesNotMatch(readFileSync(join(home, year, "routes", "bindroute.yaml"), "utf-8"), /sessions|sess-abc-123/);
    });
...
describe("the session lifecycle tack publishes", () => {
    ...
    it("lists the store with each session's state and the tacks it drove", () => {
        ...
        const json = JSON.parse(run(e, "sessions", "--json"));
...
describe("route/tack creation records the current Claude session", () => {
    ...
    it("tack start records the route once the session drives a tack", () => {
        ...
        const session = readFileSync(join(home, String(new Date().getFullYear()), "sessions", "sess-add-1.yaml"), "utf-8");
...
describe("tack doctor", () => {
    ...
    const routeFile = (slug, summary) => `id: 6e6f2f4a-0000-4000-8000-000000000001\nslug: ${slug}\n` +
        `created_at: 2026-01-01T00:00:00.000Z\nupdated_at: 2026-01-01T00:00:00.000Z\n` +
        `tacks:\n  - id: t1\n    summary: ${summary}\n    status: pending\n`;
    const overLong = (slug) => routeFile(slug, "x".repeat(600));
