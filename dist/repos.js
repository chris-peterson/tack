...
export function loadRepos() {
    if (!existsSync(REPOS_FILE))
        return {};
    return (parse(readFileSync(REPOS_FILE, "utf-8")) ?? {});
}
function saveRepos(db) {
    ensureHome();
...
export function saveReplace(db) {
    saveRepos(db);
}
function upsert(db, key, opts) {
    ...
    if (opts.local) {
        if (!entry.locals)
            entry.locals = [];
        if (!entry.locals.includes(opts.local))
            entry.locals.push(opts.local);
    }
    db[key] = entry;
}
function toMatch(key, entry) {
...
function resolveMatches(match) {
    const db = loadRepos();
    if (db[match])
        return [toMatch(match, db[match])];
    return matchByName(match);
}
function resolveOne(match) {
    const matches = resolveMatches(match);
    if (matches.length === 0)
        throw new Error(`No repo matches "${match}"`);
    if (matches.length > 1) {
        throw new Error(`"${match}" matches ${matches.length} repos: ${matches.map((m) => m.key).join(", ")}`);
    }
...
function probeLocals(parents, key) {
    ...
    if (input.srcRoot) {
        ...
        for (const [key, entry] of Object.entries(db)) {
            for (const local of probeLocals(parents, key)) {
                if (entry.locals?.includes(local))
                    continue;
                upsert(db, key, { local });
                localsAdded++;
            }
