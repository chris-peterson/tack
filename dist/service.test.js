...
import { renderPlist, renderUnit, servicePath, supervisor, wrapperPath, SERVICE_LABEL, } from "./service.js";
...
describe("service units", () => {
    ...
    it("carry the port they were installed with", () => {
        assert.match(renderPlist(wrapperPath(), 9001, "/tmp/o", "/tmp/e", PATH), /<string>9001<\/string>/);
    ...
    it("restart on their own so a reboot leaves the documents reachable", () => {
        assert.match(renderPlist(wrapperPath(), 8788, "/tmp/o", "/tmp/e", PATH), /<key>KeepAlive<\/key>/);
