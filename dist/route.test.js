...
describe("rename", () => {
    ...
    it("keeps the renamed file in the year the route was opened in", () => {
        ...
        writeFileSync(join(earlier, "old-year.yaml"), [
        ...
        assert.equal(existsSync(join(tmp, String(new Date().getFullYear()), "routes", "new-year.yaml")), false);
...
describe("removeDeliverable", () => {
    ...
    it("throws when the tack has no deliverable", () => {
        ...
        assert.throws(() => route.removeDeliverable("dlv-rm-none", "t1"), /no deliverable/i);
    });
});
describe("editTack", () => {
...
describe("recordSession", () => {
    ...
    it("finds every session that touched a route", () => {
        ...
        assert.deepEqual(route.sessionsOn("session-multi").map((s) => s.id).sort(), ["session_aaa", "session_bbb"]);
    });
    ...
    it("throws when binding a session to a tack that does not exist", () => {
        ...
        assert.throws(() => route.recordSession("session-badtack", "sess_x", "t9"), /Tack not found/);
    });
    it("refuses a session id that would not be a filename", () => {
        route.init("session-badid");
        assert.throws(() => route.recordSession("session-badid", "../escape"), /Invalid session id/);
    });
...
describe("mergeRoutes", () => {
    async function backdate(slug, createdAt) {
    ...
    it("refuses when the destination already exists", () => {
        ...
        assert.throws(() => route.mergeRoutes("mr-exists", ["mr-exists-src"]), /already exists/);
    });
    it("refuses when the destination is also a source", () => {
        route.init("mr-self");
        assert.throws(() => route.mergeRoutes("mr-self", ["mr-self"]), /cannot also be a source/);
    });
    ...
    it("carries sessions over, remapping their tack refs to the new IDs", () => {
        ...
        assert.deepEqual(route.sessionsOn("mr-sess-dst").map((s) => s.id).sort(), ["sess-1", "sess-2"]);
    });
...
describe("cross-route dependencies", () => {
    ...
    it("rejects a dependency on a route that does not exist", () => {
        ...
        assert.throws(() => route.addDependency("xr-missing", "t1", "nope/t1"), /no route nope/);
    });
    it("rejects a dependency on a missing tack in a route that exists", () => {
        ...
        assert.throws(() => route.addDependency("xr-here", "t1", "xr-there/t9"), /Dependency not found/);
    });
    it("detects a cycle that spans two routes", () => {
        ...
        assert.throws(() => route.addDependency("xr-a", "t1", "xr-b/t1"), /Circular dependency/);
    });
    ...
    it("rewrites inbound references when the depended-on route is renamed", () => {
        ...
        assert.deepEqual(route.load("xr-dependent").tacks[0].depends_on, ["xr-new/t1"]);
    });
    it("refuses to remove a tack another route depends on, and strips it with force", () => {
        ...
        assert.throws(() => route.removeTack("xr-rm-lib", "t1"), /depended on by xr-rm-app\/t1/);
...
describe("moveTack", () => {
    ...
    it("rewrites an outgoing edge into a cross-route reference", () => {
        ...
        assert.deepEqual(route.load("move-out-dst").tacks[0].depends_on, ["move-out-src/t1"]);
    ...
    it("--include-dependents leaves a staying sibling pointing across the boundary", () => {
        ...
        assert.deepEqual(route.load("move-partial-dst").tacks[0].depends_on, ["move-partial-src/t1"]);
        const sibling = route
...
describe("length limits at the command boundary (issue #49)", () => {
    it("names the field and the limit rather than a schema path", () => {
        ...
        assert.throws(() => route.editTack("bound", "t1", "x".repeat(501)), /tack summary is 501 characters; the limit is 500/);
    });
