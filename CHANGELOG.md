...
- **tack announces its own session lifecycle, so a sibling can stop reading its files.** Binding a session to a route publishes `session.started`, and `tack session end <slug> <session-id>` publishes `session.ended` with the tacks that session drove and their deliverables. A fleet view learns what a session is on and when its work closed out by listening, instead of joining against `~/.tack/routes/*.yaml` on its own. `tack session end` writes nothing — the payload is read off the route as it already stands.
- **`TACK_ANNOUNCE=0` silences a caller whose output nobody reads.** The prompt hook binds the session on every prompt and throws the output away, so it sets this; the announcement then waits for the first binding the agent makes itself, which is the one a subscriber can actually see.
...
### Added
- **Routes carry an optional `title` and `description`, set with `tack title` and `tack describe`** (issue #31). Above the tack level a route had only its slug, so a dashboard listing routes could show the name and the counts but not what the work is for. The title is a human-readable name for a slug that's terse by design, and the description is markdown prose stating the goal and why it matters. Both verbs follow the `tack group` shape: set with an argument, `--clear` to remove, bare to print the current value. Because a markdown body is multi-line, `tack describe` also reads it from `--file <path>` or from stdin with `--file -`; an empty body fails rather than storing a description the CLI would then report as unset. `tack list` prints the title after the counts, and `tack list --json` serializes the full route so a dashboard reads it there alongside every other field. Either way the title is displayed alongside the slug rather than in place of it, so the slug stays the key you type back into the CLI. `tack merge-routes` carries the first source's title and joins every source description with a `---` rule, since the merge deletes the source files and a body left behind would be unrecoverable.
