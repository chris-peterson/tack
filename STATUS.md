...
**Spec version:** v2
**Coverage:** 215 / 215 source-verified normative behaviors (100%) — 0 Partial, 0 Missing, 0 Contradicts. AGENT-08 is retired and excluded from the count

## Status by category

| Prefix | Count | Status | Notes |
|--------|------:|--------|-------|
| ROUTE-01..14 (routes; no ROUTE-10/11 — session records moved to SESS) | 12 | All Covered | `src/types.ts`, `schema/route.schema.json`, `src/route.ts`; ROUTE-04 optional fields now include `title`/`description` (`Route` in `src/types.ts`, rendered by `formatRoute` in `src/display.ts`); ROUTE-09 is the rule that a route carries no session record and that binding one writes no route file (`recordSession` in `src/route.ts`, tests in `src/route.test.ts`); ROUTE-12 states the slug pattern enforced by `schema/route.schema.json` and `assertValidSlug` (`src/route.ts`); ROUTE-13 derived route state (`routeState` in `src/route.ts`, surfaced by `formatRoute`/`formatList` in `src/display.ts` and the `--json` paths in `src/cli.ts`, never written to the file); ROUTE-14 `created_at` floor (`floorCreatedAt`, applied in `save`); STORE-01b's year rule reaches `rename`, which places the new file by `created_at` and writes it before removing the old one (`rename` in `src/route.ts`, 2 tests in `src/route.test.ts`) |
| SESS-01..09 (session document; +SESS-01a) | 10 | All Covered | `schema/session.schema.json` and `src/sessions.ts` (`sessionPath` files by start year per SESS-01, `assertValidId` the filename-safe id of SESS-03, `record` the touch-order refs and the ended_at reset of SESS-04/SESS-05, `remapRefs` the SESS-06 upkeep called from `rename`/`moveTack`/`mergeTacks`/`mergeRoutes` in `src/route.ts`); SESS-07's dangling ref resolves to nothing in `tacksOn` and in `formatRoute`'s session block (`src/display.ts`); SESS-08's filename↔`id` check in `load`, with `all` naming the file and continuing; SESS-09's scan is `onRoute`, reached from `tack status` alone via `renderRoute`'s `withSessions` in `src/cli.ts`; SESS-01a's earn-a-file rule is the early return in `record`; SESS-08's skipped file is recorded (`invalidSessions` in `src/sessions.ts`) and named with a non-zero exit by `reportInvalidSessions` in `src/cli.ts`, per [STORE-09]; 29 tests in `src/sessions.test.ts`, plus the `recordSession` / `endSession` blocks in `src/route.test.ts` and the exit-code case in `src/cli.test.ts` |
| TACK-01..08 (tacks; no TACK-04) | 7 | All Covered | `src/route.ts`; TACK-08 bare-id resolution (`normalizeTackId`), reached by `moveTack` and by the `tack tree` path lookups in `src/display.ts` (`treeData`, `formatTree`) |
| DELIVER-01..02 (deliverables) | 2 | All Covered | `src/types.ts`, `src/route.ts` |
| DEPENDS-02..07 (dependencies) | 6 | All Covered | `src/route.ts`; DEPENDS-02's store-it-bare rule holds on both writers — `addTack` maps `--depends-on` through `parseDepRef`/`formatDepRef` as `addDependency` does, tests in `src/route.test.ts` |
| LINKS-01 (links) | 1 | Covered | `src/types.ts` |
| STORE-01..12 (storage; no STORE-06 — the pins file went with the pin commands; +STORE-01a..b) | 13 | All Covered | `src/route.ts`; STORE-01 also carries the no-state-in-the-project-tree rule; STORE-07 filename↔`slug` agreement enforced in `readRoute`; STORE-08 boundary slug checks (`assertValidSlug`, called from `init`, `rename`, `setGroup`, `mergeRoutes`), tests in `src/route.test.ts`; STORE-09 read-everything commands continue past an unreadable file, name it on stderr, and exit non-zero (`scanAll`/`invalidRoutes` in `src/route.ts`, `reportInvalidRoutes` in `src/cli.ts`), with `export` (`src/backup.ts`) and the `rename`/`mergeRoutes` referer sweeps still on the strict `loadAll`; STORE-10 the length limits, held in `schema/route.schema.json` and read back by `maxLengths` in `src/schema.ts`, with the spec table guarded against drift by `src/schema.test.ts`; STORE-12 the write-before-remove order a relocation holds, so a failed rename leaves a readable route (`rename` in `src/route.ts`, tests in `src/route.test.ts`); STORE-11 boundary length checks (`assertLength`/`assertLineLength`/`assertBlockLength` in `src/route.ts`, called from `addTack`, `editTack`, `addBefore`, `addAfter`, `setTitle`, `setDescription`, `addLink`, `setDeliverable`), tests in `src/route.test.ts`/`src/cli.test.ts` |
| CLI-01..59 (commands; no CLI-30/31/39/40 — `pin`/`unpin`/`pins` removed; +CLI-08a, CLI-19a, CLI-21a..d, CLI-23a..b, CLI-36a..d, CLI-37a..b, CLI-52a..d, CLI-54a, CLI-56a..b) | 74 | All Covered | CLI-53/CLI-54/CLI-54a (`tack title` and `tack describe` show/set/clear, plus `describe`'s `--file <path>` / `--file -` body input, trailing-newline stripping, and the empty-body refusal — `title`/`describe` dispatch and `readDescriptionBody` in `src/cli.ts`, `setTitle`/`clearTitle`/`setDescription`/`clearDescription` in `src/route.ts`, tests in `src/cli.test.ts`/`src/route.test.ts`); CLI-14 carries the route `title` in the text listing (`list` in `src/route.ts`, `formatList` in `src/display.ts`), while the `--json` form serializes the full route (`loadAll` in `src/route.ts`); CLI-52c title/description carry-over on merge (`mergeRoutes` in `src/route.ts`); CLI-23a/CLI-23b (`tack find --path` path lookup and the exactly-one `--url`/`--path` selector guard — `repoKeyForCwd` in `src/repos.ts`, `findByRepoKey` in `src/route.ts`, `find` dispatch in `src/cli.ts`, tests in `src/route.test.ts`/`src/cli.test.ts`); CLI-52 (`tack merge-routes`, whole-route consolidation with chronological destination IDs, metadata/`depends_on`/session preservation, `created_at`/group defaults, external route-dep guard — `mergeRoutes` in `src/route.ts`, `merge-routes` dispatch in `src/cli.ts`, tests in `src/route.test.ts`); CLI-51 (`tack group` show/set/clear, `src/cli.ts` group case + `setGroup`/`clearGroup` in `src/route.ts`); also includes CLI-02/CLI-04 (`init`/`add` record the current session route-level via `recordSessionIfPresent`, `src/cli.ts`; CLI-04 also takes repeatable `--link "label,url"`, deduped in `addTack`), CLI-08a (`deliverable rm` clears or `--to-link`-demotes the deliverable, `src/route.ts` `removeDeliverable` + `src/cli.ts` dispatch, tests in `src/route.test.ts`/`src/cli.test.ts`), CLI-17/CLI-18 (session + `--tack` binding / `--json`), CLI-19a (`install-cli`), CLI-32..36 (depends add/rm, status set, rename, move), CLI-37 (forge note) + CLI-37a (commit-URL label derivation), CLI-38 (`--help`/`-h`/`help` + usage exit semantics, incl. subcommand-level `--help`/`-h`, `src/cli.ts`), CLI-41 (group-scoped subcommand errors on stderr, `src/cli.ts` `groupError`, `src/cli.test.ts`), CLI-42..47 (`tack repo` lookup/list/alias/prune/rebuild/rm, `src/repos.ts` + `src/cli.ts`), CLI-48 (duplicate-URL warning on attach, `src/route.ts` `findCollisions`, `src/cli.ts` `warnUrlCollision`, `src/cli.test.ts`), CLI-49/CLI-50 (`export` to stdout by default with `--out-file`/`--compress`, `import` detecting gzip-vs-plain by content, schema versioning + identity-dedup merge, `src/backup.ts` + `src/cli.ts`, `src/cli.test.ts`); CLI-04 `--link` splits at the first comma whose suffix parses as a URL (commas allowed in label and URL), CLI-15 `rm` refusal on stderr with a non-zero exit, CLI-51 group checked at the boundary per STORE-08 — all three reworded, no new IDs; CLI-37b GitLab `work_items`/epics/milestones recognition (`parseChangeRefUrl` in `src/route.ts`, `scripts/lib-url.sh`); CLI-55 top-level error handler (`fail` in `src/cli.ts`, `TACK_DEBUG` restores the stack, tests in `src/cli.test.ts`); CLI-56/CLI-56a/CLI-56b `tack reconcile` — candidate selection, forge merge-timestamp `done_at`, and the gh/glab-only network path (`reconcile`/`mergeState` in `src/reconcile.ts`, `reconcile` dispatch in `src/cli.ts`, 8 tests in `src/reconcile.test.ts` driving an injected probe); CLI-57 `tack doctor [--json]` reports every route file that will not load and exits non-zero when any does (`doctor` in `src/route.ts`, the `doctor` dispatch in `src/cli.ts`, the completion entry in `src/completions.ts`, tests in `src/cli.test.ts`/`src/route.test.ts`); CLI-58 `tack session end` stamps the session's `ended_at` and announces its work closing out, writing nothing for a session the store never saw (the `session end` branch in `src/cli.ts`, `endSession`/`sessionWork` in `src/route.ts`, `end` in `src/sessions.ts`, the completion entry in `src/completions.ts`, tests in `src/cli.test.ts`); CLI-59 `tack sessions [--json]` lists the store newest first (the `sessions` dispatch in `src/cli.ts`, `all` in `src/sessions.ts`, `formatSessions` in `src/display.ts`, tests in `src/cli.test.ts`); CLI-36 now moves the tack a bare `<N>` names (`moveTack` reads the canonical id from `findTack`, test in `src/route.test.ts`); CLI-20/CLI-21d completion helpers read the year-partitioned store (`_tack_route_file` in `src/completions.ts`, 5 tests in `src/completions.test.ts` driving zsh with `-f`); CLI-21a/CLI-24/CLI-28/CLI-41 reworded off the retired todo surface, and CLI-49 states the `schemaVersion` `archiveVersion` actually stamps |
| AGENT-01..11 (agent; AGENT-08 retired) | 10 | All Covered | `guides/routes.md` carries the reasoning (AGENT-03 resolution procedure, AGENT-04..08 recording and completion, AGENT-09/AGENT-11 session→tack binding and the existing-vs-emerging derivation, AGENT-10 the branch-named-for-the-slug rule); the reminder text that invokes it is `url_nudges` in `scripts/lib-url.sh` and the resolved-route line in `hooks/session-nudge.sh`. AGENT-05 names the three-positional `tack deliverable` grammar [CLI-08] requires, and AGENT-08 is struck in place — the post-work todo surface it described is gone. AGENT-01 and AGENT-02 reworded with the removal of the `tack` skill — the CLI stays the only write path, and the agent reads the named route on demand instead of loading every active route at session start |
...
| SERVE-01..17 (document server; +SERVE-02a) | 18 | All Covered | SERVE-02 defers a tack's own document to SERVE-17 rather than forbidding it; SERVE-04's 500 is scoped to the requested route's own document and reaches a tack path inside it (`handle`'s tackMatch branch, test in `src/serve.test.ts`); the index and group pages render what they could read and name the rest (`scanAll` + the `renderIndex` banner in `src/serve.ts`). `src/serve.ts` (`serve`, `handle`, `renderIndex`/`renderRoute`/`renderGroup`, `prefersJson`, `loopbackHost`, `sameOrigin`, `markdown`, `hyperlinkBase`) and `src/service.ts` (`install`/`uninstall`/`status`, `renderPlist`/`renderUnit`); SERVE-02a group documents list route cards with their in-flight tacks and cross-route edges, linking out rather than anchoring in place (`renderGroup`/`routeCard`); SERVE-09 OSC 8 links rendered by `formatRoute`/`formatTack` in `src/display.ts` from the base `src/cli.ts` resolves; SERVE-12/SERVE-13 the edit form and its `Origin` check; SERVE-15 carries `TACK_HOME` into the unit (`serviceHome` + `renderPlist`/`renderUnit` in `src/service.ts`) and SERVE-16 names the store it read on the toolbar home link and the empty index (`storeRoot` in `src/route.ts`, `page`/`renderIndex` in `src/serve.ts`) and SERVE-17 serves a tack on its own with its deliverable set apart from its links (`renderTack`/`landed`/`refs` and the `/route/<slug>/<tack-id>` branch in `src/serve.ts`); 53 tests in `src/serve.test.ts`, 12 in `src/service.test.ts`. SERVE-08's launchd/systemd load-unload round trip is exercised by hand, not by the suite |
| COMPAT-01..07 (compatibility; +COMPAT-06a) | 8 | All Covered | The contract itself is prose in `SPEC.md`, summarized in `README.md`; the parts with a gate: COMPAT-02's closed schema (`additionalProperties: false` throughout `schema/route.schema.json` and `schema/session.schema.json`), COMPAT-05's grammar snapshot (`spec/cli-usage.txt` vs `tack --help`, asserted in `src/cli.test.ts`), COMPAT-06's archive version guard (`SCHEMA_VERSION` in `src/backup.ts`, refusal tested in `src/cli.test.ts`), COMPAT-06a's conversion rule, whose gate is the refusal a predating file gets (`validate` in `src/schema.ts`, tested in `src/schema.test.ts`). COMPAT-01/03/04 are declarations a reviewer applies, with no code path to point at |

## Audit history

### 2026-09-21 — Divergences found and resolved (spec-sync)

Coverage 215/215 → 215/215: +1 ID (STORE-12), AGENT-08 retired, and the `tack context`
requirements carved out to issue #81 rather than left in the spec as a standing gap. The prior
100% did not hold — a full spec ⇄ code pass found 8 Contradicts and 7 Partial. Seven were
code defects, now fixed with tests: `rename` corrupting a route filed under an earlier year
(STORE-01b), `tack move` no-op'ing on a bare id (TACK-08, CLI-36), `--depends-on` storing a
self-qualified edge (DEPENDS-02), completion helpers reading a pre-year-partition store
(CLI-20, CLI-21d), the start nudge never suppressing on `/tack:start` (HOOK-04b),
`tack sessions` exiting zero over a file it skipped (SESS-08), and a tack path answering 404
where its route answers 500 (SERVE-04). Six were stale spec text, now true: SERVE-02,
CLI-49, CLI-21a, CLI-24, CLI-28, CLI-41. AGENT-05's example, five stale cross-references,
and the EVENTS row missing from the category table were corrected with them.

### 2026-09-21 — Coverage refresh (spec-status)

+15 IDs (SESS-01..09 with SESS-01a, CLI-59, COMPAT-06a, SERVE-15/16/17), −2 (ROUTE-10 and ROUTE-11, which
described the session records a route no longer holds; ROUTE-09 now states their
absence), coverage 203 → 215, all Covered, no needs-decision rows. SESS is a new
category: a session has no direct relationship with a route, so the copy a route
used to hold was one per route its tacks reached, free to disagree with the
others. COMPAT-06a states the conversion rule this change ships under: the
store is rewritten in place, and a file predating the rewrite is refused with a
message saying what changed.

Four rows' counts were recomputed against the spec in the same pass and had
drifted from earlier changes rather than from this one — TODO-01..05 retired
with `before`/`after` in 1.7 and its row is gone, DEPENDS is 02..07, STORE
gained 01a..b, HOOK gained 01a..b. Their statuses are as the 2026-09-02 audit
left them; only the counts were re-derived.

### 2026-09-02 — Coverage refresh (spec-status)

+7 IDs (EVENTS-01..06, CLI-58), coverage 196 → 203, all Covered, no
needs-decision rows. EVENTS is a new category: it covers tack as a publisher,
where HOOK-09/HOOK-10 cover the reader side of the same contract. EVENTS-03 and
EVENTS-04 record why the once-per-session gate sits on the announcement rather
than on the binding — HOOK-04's bind reaches no subscriber, so gating on the
first binding would publish into silence.
...
+1 ID, coverage 189 → 190, all Covered, no needs-decision rows. HOOK-04d states
the resolved-route line the prompt hook now emits. AGENT-01 and AGENT-02 were
reworded in place with the removal of the `tack` skill: the agent's reasoning
moved to `guides/routes.md`, which the hook reminders name by path, and the
install door moved to `/tack:install-tack` (HOOK-01).

### 2026-08-25 — Coverage refresh (spec-status)

−2 IDs, coverage 191 → 189, all Covered, no needs-decision rows. SESSION-11 and
SESSION-12 retired with the worktree reap; 1.4.0 added them, taking coverage
189 → 191 in a release with no ledger pass of its own. SESSION-01..10 re-verified
against `skills/start/SKILL.md` and `skills/end/SKILL.md`.
