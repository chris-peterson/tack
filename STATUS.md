...
**Spec version:** v1
**Coverage:** 191 / 191 source-verified normative behaviors (100%) — 0 Partial, 0 Missing, 0 Contradicts
...
| SESSION-01..12 (open and close) | 12 | All Covered | `skills/start/SKILL.md` (SESSION-01..05: the gate, slug derivation via `skills/start/scripts/parse-start-args.py`, fresh-base branch cut, and the no-`tack session` rule) and `skills/end/SKILL.md` (SESSION-06..09: the non-sequencing boundary, the step-5 output table, the dead-end bookkeeping; SESSION-11/12: the step-3 worktree reap, its three conditions and non-forcing commands, and the table row an unreapable tree earns); SESSION-10 stated in both — neither skill writes an external session label |
