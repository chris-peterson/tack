...
let repos: typeof import("./repos.js");
...
describe("normalizeGitRemote (REPO-02)", () => {
  it("strips scheme and .git from an HTTPS remote", () => {
    assert.equal(
      repos.normalizeGitRemote("https://github.com/chris-peterson/anchor.git"),
  ...
  it("normalizes an scp-style SSH remote to the same key", () => {
    assert.equal(
      repos.normalizeGitRemote("git@github.com:chris-peterson/anchor.git"),
      "github.com/chris-peterson/anchor",
    );
  });
  ...
  it("normalizes an ssh:// remote with userinfo", () => {
    assert.equal(
      repos.normalizeGitRemote("ssh://git@github.com/chris-peterson/anchor.git"),
      "github.com/chris-peterson/anchor",
    );
  });
  ...
  it("collapses HTTPS and SSH forms to one identity", () => {
    assert.equal(
      repos.normalizeGitRemote("https://github.com/chris-peterson/anchor"),
      repos.normalizeGitRemote("git@github.com:chris-peterson/anchor.git"),
    );
  });
  ...
  it("handles a self-hosted GitLab remote with nested groups", () => {
    assert.equal(
      repos.normalizeGitRemote("https://gitlab.example.com/group/sub/repo.git"),
...
describe("repoKeyFromForgeUrl (REPO-06)", () => {
  it("extracts host/org/repo from a GitHub PR URL", () => {
    assert.equal(
      repos.repoKeyFromForgeUrl("https://github.com/chris-peterson/anchor/pull/42"),
      "github.com/chris-peterson/anchor",
    );
  });
  ...
  it("extracts from a GitHub issue and commit URL", () => {
    assert.equal(
      repos.repoKeyFromForgeUrl("https://github.com/org/repo/issues/3"),
      "github.com/org/repo",
    );
    assert.equal(
      repos.repoKeyFromForgeUrl("https://github.com/org/repo/commit/abc1234"),
      "github.com/org/repo",
    );
  });
