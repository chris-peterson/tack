...
export function assertValidId(id: string): void {
  if (SESSION_ID.test(id) && id.length <= 200) return;
  throw new Error(
    `Invalid session id: ${id} (letters, digits, dot, underscore, and hyphen only)`,
  );
}
...
function sessionPath(id: string, startedAt?: string): string {
...
export function record(id: string, slug: string, tackId?: string): Session | null {
  assertValidId(id);
  ...
  if (!existing && !tackId) return null;
  ...
  if (!session.routes) session.routes = [];
  if (!session.routes.includes(slug)) session.routes.push(slug);
...
export function end(id: string): Session | null {
  assertValidId(id);
  const session = load(id);
...
export function tacksOn(id: string, slug: string): string[] {
  const prefix = `${slug}/`;
...
export function onRoute(slug: string): Session[] {
...
export function remapRefs(
  ...
  slugs: Map<string, string | null> = new Map(),
): string[] {
  if (map.size === 0 && slugs.size === 0) return [];
  ...
  for (const session of all()) {
    ...
    if (session.tacks?.length) {
      ...
      if (next.length) session.tacks = next;
      else delete session.tacks;
    }
    ...
    if (session.routes?.length) {
      ...
      if (next.length) session.routes = next;
      else delete session.routes;
    }

    if (JSON.stringify([session.routes, session.tacks]) === before) continue;
    save(session);
