...
function tackCard(t: Tack, opts: { href?: string; anchor?: boolean } = {}): string {
...
function refs(t: Tack, opts: { depends?: (id: string) => string } = {}): string {
...
export function renderTack(r: Route, t: Tack): string {
  const done = t.status === "done";
  const dep = (entry: string) => {
...
<p class="sub">${esc(r.slug)}/${esc(t.id)}${
    ...
    t.done_at ? ` &middot; done ${esc(String(t.done_at).slice(0, 10))}` : ""
  }</p>
${deliverable}
