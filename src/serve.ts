...
export function handle(req: IncomingMessage, res: ServerResponse): void {
  if (!loopbackHost(req)) return send(res, 403, page("tack", "<h1>403</h1>"));
  if (req.method !== "GET") return send(res, 405, page("tack", "<h1>405</h1>"));
  ...
  try {
    ...
    return send(res, 500, page("tack", `<h1>500</h1><p>${esc((e as Error).message)}</p>`));
  }

  if (path === "/") return send(res, 200, page("tack", renderIndex(routes)));

  const routeMatch = path.match(/^\/route\/([^/]+)\/?$/);
  if (routeMatch) {
    const r = routes.find((x) => x.slug === routeMatch[1]);
    if (!r) return send(res, 404, page("tack", `<h1>404</h1><p>No route ${esc(routeMatch[1])}.</p>`));
