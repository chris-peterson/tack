...
describe("the session lifecycle tack publishes", () => {
  ...
  function store(): { home: string; e: NodeJS.ProcessEnv } {
    const home = mkdtempSync(join(tmpdir(), "tack-publish-"));
    const e: NodeJS.ProcessEnv = { ...process.env, TACK_HOME: home, TMPDIR: home };
    delete e.CLAUDE_CODE_SESSION_ID;
    return { home, e };
  }

  function run(e: NodeJS.ProcessEnv, ...args: string[]): string {
  ...
  it("announces session.started on the first binding, with the tack it bound", () => {
    ...
    assert.match(
      out,
      /^codes\.bridgeai\.tack\/session\.started \{"session":"sess-1","route":"pub","tack":"t1"\}$/m,
    );
  });
  ...
  it("announces session.ended with the tacks the session drove and their deliverables", () => {
    ...
    assert.match(
      out,
      /^codes\.bridgeai\.tack\/session\.ended \{"session":"sess-1","route":"pub","tacks":\["t1"\],"deliverables":\["https:\/\/github\.com\/o\/r\/pull\/7"\]\}$/m,
    );
  });
...
describe("tack add --link splits label from url (issue #28)", () => {
  it("keeps a comma inside the label", () => {
    ...
    const json = JSON.parse(runFail(["list", "--json"]).stdout) as Array<{
      slug: string;
      tacks: Array<{ links?: Array<{ label: string; url: string }> }>;
    }>;
...
describe("an unreadable route file does not take the listing down with it", () => {
  ...
  function storeWithOneBadFile(): NodeJS.ProcessEnv {
  ...
  function run(e: NodeJS.ProcessEnv, args: string[]) {
  ...
  for (const args of [["list"], ["recent"], ["tree"], ["status"]]) {
    ...
    it(`${args.join(" ")} names the file it left out, on stderr`, () => {
      ...
      assert.match(r.stderr, /must NOT have more than 500 characters/);
    });
  ...
  it("list --json emits a parseable array of the readable routes on stdout", () => {
    ...
    const parsed = JSON.parse(r.stdout);
    assert.deepEqual(parsed.map((x: { slug: string }) => x.slug), ["good-one"]);
  });
...
describe("tack doctor", () => {
  function storeWith(files: Record<string, string>): NodeJS.ProcessEnv {
  ...
  const routeFile = (slug: string, summary: string) =>
    `id: 6e6f2f4a-0000-4000-8000-000000000001\nslug: ${slug}\n` +
    `created_at: 2026-01-01T00:00:00.000Z\nupdated_at: 2026-01-01T00:00:00.000Z\n` +
    `tacks:\n  - id: t1\n    summary: ${summary}\n    status: pending\n`;

  const overLong = (slug: string) => routeFile(slug, "x".repeat(600));
  ...
  it("reports the route, the path, and the rule for each violation", () => {
    ...
    assert.match(r.stdout, /\/tacks\/0\/summary/);
    assert.match(r.stdout, /must NOT have more than 500 characters/);
  });
  ...
  it("emits the same report as JSON", () => {
    ...
    assert.match(parsed.invalid[0].errors[0], /must NOT have more than 500 characters/);
  });
});

describe("text is bounded at the command boundary (issue #49)", () => {
  // Not the raw ajv path (`/tacks/0/summary`) the write path used to surface:
  // the caller supplied a summary, so the message names the summary.
  it("names the field and the limit rather than a schema path", () => {
    ...
    assert.equal(r.status, 1);
    assert.match(r.stderr, /tack summary/);
    assert.match(r.stderr, /500/);
    assert.doesNotMatch(r.stderr, /\/tacks\/\d+\//);
  });

  it("accepts a summary at the limit", () => {
    runFail(["init", "summary-limit-ok"]);
    assert.equal(runFail(["add", "summary-limit-ok", "x".repeat(500)]).status, 0);
  });

  it("bounds a route title against its own smaller limit", () => {
    runFail(["init", "title-limit"]);
    const r = runCapture(["title", "title-limit", "x".repeat(201)]);
    assert.equal(r.status, 1);
