...
describe("content negotiation", () => {
  ...
  it("defaults to the document for a browser's Accept", () => {
    assert.equal(
      serve.prefersJson("text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"),
      false,
    );
  });
  ...
  it("serves a route as JSON matching the CLI's --json shape", async () => {
    ...
    await withServer(async (base) => {
      ...
      const body = (await res.json()) as { slug: string; state: string; tacks: unknown[] };
  ...
  it("serves the index and a group as JSON arrays", async () => {
    ...
    await withServer(async (base) => {
      const all = (await (await fetch(`${base}/`, { headers: { accept: "application/json" } })).json()) as unknown[];
      assert.equal(all.length, 2);
      const group = (await (
        await fetch(`${base}/group/team`, { headers: { accept: "application/json" } })
      ).json()) as unknown[];
...
describe("editing a route from the page", () => {
  async function post(base: string, slug: string, body: string, headers: Record<string, string> = {}) {
...
describe("description markdown", () => {
  async function descHtml(base: string, slug: string): Promise<string> {
