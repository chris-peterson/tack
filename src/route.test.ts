...
describe("findByRepoKey (CLI-23a)", () => {
  it("matches deliverables and links belonging to the repo key, across routes", () => {
    ...
    assert.deepEqual(
      matches.map((m) => `${m.slug}/${m.tackId}:${m.match}`).sort(),
      ["fbk-a/t1:deliverable", "fbk-a/t2:link"],
    );
  });
...
describe("slug validation at the command boundary", () => {
  it("init rejects a non-conforming slug by naming the rule", () => {
    assert.throws(
      () => route.init("MyRoute"),
      /Invalid slug: MyRoute \(lowercase letters, digits, and inner hyphens only\)/,
    );
  });
...
describe("filename and internal slug must agree", () => {
  it("load refuses a route file whose slug disagrees with its filename", () => {
    ...
    assert.throws(
      () => route.load("other-name"),
      /declares slug 'real-slug' — rename the file to real-slug\.yaml, or set slug: other-name inside it/,
    );
  });
...
describe("length limits at the command boundary (issue #49)", () => {
  ...
  it("bounds a link url", () => {
    ...
    assert.throws(
      () => route.addLink("bound-url", "t1", "label", `https://x/${"y".repeat(2048)}`),
      /link url is \d+ characters; the limit is 2048/,
    );
  });
});
