...
function renderRoute(
  r: Route,
  opts: { linkBase?: string | null; withSessions?: boolean } = {},
): string {
...
function readDescriptionBody(file: string): string {
...
function run(): void {
  ...
  switch (command) {
    ...
    case "add": {
      ...
      const links = (values.link as string[] | undefined)?.map((spec) => {
        for (let i = spec.indexOf(","); i >= 0; i = spec.indexOf(",", i + 1)) {
          ...
          if (URL.canParse(url)) return { label: spec.slice(0, i), url };
        }
        console.error(
          `Invalid --link "${spec}": expected "label,url" where url is absolute ` +
    ...
    case "serve": {
      ...
      const port = serveValues.port ? parseInt(serveValues.port as string, 10) : serve.DEFAULT_PORT;
    ...
    case "describe": {
      ...
      if (!describeSlug) usage();
      const describeFile = describeValues.file as string | undefined;
    ...
    case "export": {
      ...
      if (outFile) {
        ...
        console.error(
          `exported ${counts.routes} routes, ${counts.repos} repos → ${outFile} ` +
