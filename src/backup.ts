...
export function parseArchive(buf: Buffer): Archive {
  ...
  try {
    ...
    throw new Error(`archive is not valid JSON (${(e as Error).message})`);
  }
  ...
  if (a.schemaVersion > SCHEMA_VERSION) {
    throw new Error(
      `archive schemaVersion ${a.schemaVersion} is newer than this tack supports (${SCHEMA_VERSION}); upgrade tack`,
    );
  }
...
function identity(t: Tack): string {
...
export function applyImport(
  ...
  for (const incoming of archive.routes) {
    if (!route.routeExists(incoming.slug)) {
      ...
      if (!opts.dryRun) route.writeRoute(incoming);
      continue;
    }
    const local = route.load(incoming.slug);
    const byIdentity = new Map<string, string>();
    for (const t of local.tacks) byIdentity.set(identity(t), t.id);
