...
<!-- Generated by shipyard from spec/v1/cli.yml — do not edit by hand. -->

Tack ids display as `t<N>`. Anywhere a command takes a `<tack-id>`,
`<dep-id>`, or a `--depends-on` entry, the bare number works too — `t7` and
`7` both resolve to the same tack.

Every command `tack` documents in its own help output. It records what the CLI documents, which is not necessarily everything it accepts.
...
Making a route, and finding your way around the ones you have.
...
tack group <slug> [<group>] [--clear]
    (no group: show current; --clear: ungroup)
```
...
tack title <slug> [<text>] [--clear]
    (no text: show current; --clear: remove)
```
...
tack tree [<path>] [-d <depth>] [--json]
    (path supports glob: */*/deliverable)
```
...
Every deliverable — the glob matches across levels.

```bash
tack tree '*/*/deliverable'
```

Drill into one route, two levels deep.

```bash
tack tree my-route -d 2
```
...
tack find --url <url> [--json]
    Find tacks referencing a URL (in any deliverable or link)
tack find --path [<dir>] [--json]
    Find routes covering a repo checkout (default cwd)
```
...
Which tack references this URL, in any deliverable or link.

```bash
tack find --url https://github.com/owner/repo/pull/42
```

Which routes cover the checkout you're standing in.
...
What a tack produced, and how it gets closed.

### `tack deliverable`

```text
tack deliverable <slug> <tack-id> <url> [--label <text>] [--force]
    (label auto-derived from url by default)
tack deliverable rm <slug> <tack-id> [--to-link]
    (clear the deliverable, or --to-link to demote it into links)
```
...
tack reconcile [<slug>] [--dry-run]
    Close tacks whose deliverable has merged (asks the git forge)
```
...
Ask the forge which deliverables have merged, without closing anything.

```bash
tack reconcile --dry-run
```
...
Serving route documents over HTTP, optionally supervised.

### `tack serve`

```text
tack serve [--port <n>]
    Serve route documents on 127.0.0.1 (Ctrl-C to stop)
tack serve install|uninstall|status [--port <n>]
    Manage the supervised server
```
...
#### `tack serve install|uninstall|status`
...
The checkout-to-route mapping routes are resolved against.

### `tack repo`

```text
tack repo [<partial>] [--json]
    Look up repo remote(s) by name; no arg lists all
tack repo alias <match> <alias>
    Add a custom name to a repo
tack repo prune
...
```text
tack doctor [--json]
    Report route files that will not load
```
...
tack export [--out-file <path>] [--compress]
    Dump a backup to stdout (routes + repos)
```
...
tack import <file> [--merge|--replace] [--dry-run]
    Merge (default) or restore a backup
```
